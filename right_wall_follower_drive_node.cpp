...
public:
      ...
      : Node("right_wall_follower_drive_node")
  {
    ...
    update_timer_ = this->create_wall_timer(80ms, std::bind(&right_wall_follower::update_callback, this));
...
private:
  ...
  { 
    ...
    for (int i = 0; i < count; i++)
    {
      ...
      ***************************************/
      ...
      { 
        ...
        {
          if (front_view <= 1.7) //SMALLER is better, too small makes lag response
          {
    ...
    //printf("total RIGHT is %f & total LEFT is %f\n", total_right, total_left);
  ...
  void update_callback()
  {
    ...
    float front_dist = 1.5; //Set to LOW for PID TEST Only..........

    double error;
    double pid; 
    
    kp_ = 1.5; // 3.27 How fast the car to steer to the desired point. Higher is better but to much will lead to spinning
    ki_ = 0;	//0.75 When there is an obstacle distraction. It is used to correct the lane ofset to its desired point. Too much will makes it oscilated in the desired point.
    kd_ = 20.0; //20.18 Damp the car to steer to the desired point. Too high lead to overdamp which makes it too long to get to the desired point. Too low makes it oscilate
