...
private:
  ...
  { 
    ...
    if (front_view > range_max)
    {
      front_view = range_max;
    }
    
    if (front_view < range_max && front_view > range_min)
    {
      printf("\n");
    } 
    ...
    for (int i = 0; i < count; i++)
    {
      ...
      ***************************************/
      ...
      { 
        ...
        if (degree > 1.0 && degree <= 80.0)
        {
          total_left += laser_ranges[i];
          
          if (laser_ranges[i] <= 2.0)
          {
            gap_index_left += i;
            gap_left++;
          }
  ...
  void update_callback()
  {
    ...
    if (dright < range_max && dleft < range_max)
    {
    ...
    kp_ = 4;	//How fast the car to steer to the desired point. Higher is better but to much will lead to spinning
    ki_ = 0.5;	//When there is an obstacle distraction. It is used to correct the lane ofset to its desired point. Too much will makes it oscilated in the desired point.
    kd_ = 1.0;	//Damp the car to steer to the desired point. Too high lead to overdamp which makes it too long to get to the desired point. Too low makes it oscilate
